...
export class ProductCard extends ProductCardLink {
  ...
  #setupHoverVideo() {
    ...
    const gallery = this.refs.cardGallery || this.querySelector('.card-gallery') || this;
    const wrapper = gallery?.querySelector('[data-hover-video-wrapper]') || this.querySelector('[data-hover-video-wrapper]');
  ...
  previewImage(event) {
    ...
    const targetIndex = slideshow.slides && slideshow.slides.length > 1 ? 1 : slideshow.nextIndex;
    slideshow.select(targetIndex, undefined, { animate: true });
  ...
  resetImage(event) {
    ...
    if (slideshow && slideshow.hasAttribute('no-controls')) {
      const scroller = slideshow.querySelector('slideshow-slides');
    ...
    const targetIndex = this.#previousSlideIndex != null ? this.#previousSlideIndex : 0;
    if (slideshow) {
      slideshow.select(targetIndex, undefined, { animate: true });
    }
    this.#previousSlideIndex = null;
